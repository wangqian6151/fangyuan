...
class MongoPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        print('mongodb status', self.db[item.collection].find({"number": item['number']}).count())
        self.logger.debug('mongodb status' + str(self.db[item.collection].find({"number": item['number']}).count()))
        if self.db[item.collection].find({"number": item['number']}).count():  # 如果找到number,则只更新
            self.db[item.collection].update({"number": item['number']}, dict(item), upsert=True)
