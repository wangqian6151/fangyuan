...
from fangyuan.items import GanjiShoprentalItem


class GanjishoprentalSpider(scrapy.Spider):
    name = 'ganjishoprental'
    allowed_domains = ['sz.ganji.com']
    start_urls = ['http://sz.ganji.com/fang6/']

    custom_settings = {
        'LOG_FILE': 'log_ganjishoprental.txt',
    }
    ...
    def parse_list(self, response):
        ...
        for i in div:
            ...
            if i.css('.unit::text').extract_first():
                item['month_price'] = i.css('.num::text').extract_first() + i.css('.unit::text').extract_first()
            else:
    ...
    def parse_details(self, response):
        ...
        item['width'] = response.css(' ul.er-list.f-clear > li:nth-child(3) > span.content::text').extract_first()
        item['lease'] = '/'.join(i.strip() for i in response.css('ul.er-list.f-clear > li:nth-child(4) > span.content::text').extract_first().split('/'))
        item['depth'] = response.css(' ul.er-list.f-clear > li:nth-child(5) > span.content::text').extract_first()
        item['height'] = response.css(' ul.er-list.f-clear > li:nth-child(7) > span.content::text').extract_first()
        self.logger.debug('parse_details street:' + response.css(' ul.er-list.f-clear > li:nth-child(8) > span.content::text').extract_first())
        if response.css(' ul.er-list.f-clear > li:nth-child(8) > span.content::text').extract_first():
            item['street'] = response.css(' ul.er-list.f-clear > li:nth-child(8) > span.content::text').extract_first().strip(' - ')
        item['total_floor'] = response.css(' ul.er-list.f-clear > li:nth-child(9) > span.content::text').extract_first().strip()
        self.logger.debug('parse_details shop_type:' + response.css('ul.er-list.f-clear > li:nth-child(12) > span.content::text').extract())
        if response.css('ul.er-list.f-clear > li:nth-child(12) > span.content::text').extract():
            item['shop_type'] = response.css('ul.er-list.f-clear > li:nth-child(12) > span.content::text').extract()[1].strip(' - ')
        # 获取经纬度方法一
